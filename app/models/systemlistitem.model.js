...
systemlistitem.findById = (id, result) => {
  sql.query(
    `SELECT sys.listItemId, sys.listid, sys.listItemName, sysli.listName as listName FROM systemlistitem sys inner join systemlist sysli on sys.listid = sysli.listId WHERE listItemId = ${id}`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
...
systemlistitem.create = (newsystemlistitem, result) => {
  sql.query(
    "INSERT INTO systemlistitem SET ?",
    newsystemlistitem,
    (err, res) => {
      if (err) {
        result(err, null);
...
systemlistitem.remove = (id, result) => {
  sql.query(
    "DELETE FROM systemlistitem WHERE listItemId = ?",
    id,
    (err, res) => {
      if (err) {
        result(null, err);
